...
def start_game():
  ...
  if not choices[0]:
    narrate_sleep("You see a rusty hole, in which your scuba barely fits in the crack.|But were able to get in.|Now you see two corridors.".split("|"))
    choices[1] = choose_place("Which way would you go?", "1 for left or 2 for right")
    if not choices[1]:
      narrate_sleep("You selected to dive through the left corridor.|At the end of the corridor, you now see a hole in the floor and a casket.".split("|"))
      ...
      narrate_sleep("You selected to dive through the right corridor.|After diving for a few meters more, you now see two strange rooms.".split("|"))
      choices[2] = choose_place("Which room would you get in?", "1 or 2")
  else:
    narrate_sleep("You see a round window, you managed to squeeze in.|The room you got into appears to be an officer's quarters.".split("|"))
    choices[1] = choose_place("Do you want to investigate the room further?", "1 for yes or 2 for no")
    if not choices[1]:
      narrate_sleep("You decided to investigate the room further.|You then discovered that the room have a secret basement.".split("|"))
      choices[2] = choose_place("Do you want to enter?","1 for yes or 2 for no")
    else:
      narrate_sleep("After you left the room, you saw an item that looks like a chest.".split("|"))
...
def get_outcomes():
  nodes = [[["After going in the hole...|You got sucked in by an unknown force and died.|Game Over...","The casket is a home of a sea vampire and he bit you.|You then runned out of blood and died.|Game Over..."],["The room you went into contains bombs from world war 2.|Booooom!!!|Game Over...","The room you went into is the home of an ancient sea dragon.|He gobbles you down in one bite.|Game Over..."]],[["After getting in the secret basement...|The door suddenly closed causing you to be locked up until you die...|Game Over...","After leaving the the secret basement alone...|A giant octupus came and stripped you out causing you to drown.|Game Over..."],["You opened the treasure chest...|There are interesting items inside...|A venomous snake then bit you causing you to die.|Game Over...","After leaving the chest alone...|A shark suddenly came and ate you.|Game Over..."]]]
